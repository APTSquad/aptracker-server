...
namespace Microsoft.Identity.Web
{
    ...
    public class TokenAcquisition : ITokenAcquisition
    {
        ...
        /// </summary>
        private readonly string[] _scopesRequestedByMsalNet =
        {
        ...
        ///     <a href='https://docs.microsoft.com/azure/active-directory/develop/v2-oauth2-auth-code-flow'>
        ///         Authorization code
        ///         flow grant flow
        ///     </a>
        ///     in a web app.
        ///     It uses the code to request an access token from the Microsoft Identity platform and caches the tokens and an entry
        ///     about the signed-in user's account in the MSAL's token cache.
        ///     The access token (and refresh token) provided in the <see cref="AuthorizationCodeReceivedContext" />, once added to
        ///     the cache, are then used to acquire more tokens using the
        ///     <a href='https://docs.microsoft.com/azure/active-directory/develop/v2-oauth2-on-behalf-of-flow'>on-behalf-of flow</a>
        ...
        ///     cases where a given account is guest in other tenants, and you want to acquire tokens for a specific tenant, like
        ///     where the user is a guest in
        /// </param>
        ...
        ///     in a method called by JwtBearerOptions.Events.OnTokenValidated, the
        ///     HttpContextExtensions.StoreTokenUsedToCallWebAPI method
            ...
            string tenant = null)
        {
            ...
            {
                accessToken =
                    await GetAccessTokenOnBehalfOfUserFromCacheAsync(application, CurrentHttpContext.User, scopes,
                            tenant)
                        .ConfigureAwait(false);
            }
            catch (MsalUiRequiredException ex)
            {
                ...
                var validatedToken = CurrentHttpContext.GetTokenUsedToCallWebAPI();
                ...
                if (validatedToken != null)
                {
                    ...
                    var tokenUsedToCallTheWebApi = validatedToken.InnerToken == null
                        ? validatedToken.RawData
        ...
        /// </summary>
        /// <param name="context">
        ///     RedirectContext passed-in to a <see cref="OnRedirectToIdentityProviderForSignOut" />
        ///     Openidconnect event
        /// </param>
        /// <returns></returns>
        public async Task RemoveAccountAsync(RedirectContext context)
        {
            ...
            var account = await app.GetAccountAsync(context.HttpContext.User.GetMsalAccountId()).ConfigureAwait(false);
        ...
        ///     Replies to the client through the HttpReponse by sending a 403 (forbidden) and populating wwwAuthenticateHeaders so
        ///     that
        ...
        public void ReplyForbiddenWithWwwAuthenticateHeader(IEnumerable<string> scopes,
            MsalUiRequiredException msalServiceException)
        {
            ...
            var proposedAction = "consent";
            if (msalServiceException.ErrorCode == MsalError.InvalidGrantError)
                if (AcceptedTokenVersionMismatch(msalServiceException))
                    throw msalServiceException;

            var consentUrl = $"{application.Authority}/oauth2/v2.0/authorize?client_id={_azureAdOptions.ClientId}"
            ...
            var scheme = "Bearer";
            var v = new StringValues($"{scheme} {parameterString}");

            var httpResponse = CurrentHttpContext.Response;
            var headers = httpResponse.Headers;
            httpResponse.StatusCode = (int) HttpStatusCode.Forbidden;
            if (headers.ContainsKey(HeaderNames.WWWAuthenticate)) headers.Remove(HeaderNames.WWWAuthenticate);
        ...
        {
            if (application == null) application = BuildConfidentialClientApplication();
            return application;
        }
        ...
        private IConfidentialClientApplication BuildConfidentialClientApplication()
        {
            ...
            var authority = $"{applicationOptions.Instance}{applicationOptions.TenantId}/";
            ...
            string tenant)
        {
            ...
            return await GetAccessTokenOnBehalfOfUserFromCacheAsync(application, accountIdentifier, scopes, loginHint,
        ...
        private async Task<string> GetAccessTokenOnBehalfOfUserFromCacheAsync(
            ...
            string tenant)
        {
            if (accountIdentifier == null) throw new ArgumentNullException(nameof(accountIdentifier));
