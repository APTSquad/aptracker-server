...
namespace Microsoft.Identity.Web.Resource
{
    public static class ScopesRequiredHttpContextExtensions
    {
        ...
        {
            if (acceptedScopes == null) throw new ArgumentNullException(nameof(acceptedScopes));
            var scopeClaim = context?.User?.FindFirst("http://schemas.microsoft.com/identity/claims/scope");
            if (scopeClaim == null || !scopeClaim.Value.Split(' ').Intersect(acceptedScopes).Any())
            {
                context.Response.StatusCode = (int) HttpStatusCode.Unauthorized;
                var message =
                    $"The 'scope' claim does not contain scopes '{string.Join(",", acceptedScopes)}' or was not found";
