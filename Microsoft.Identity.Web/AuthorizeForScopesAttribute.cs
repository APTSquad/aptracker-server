...
namespace Microsoft.Identity.Web
{
    ...
    public class AuthorizeForScopesAttribute : ExceptionFilterAttribute
    {
        ...
        {
            var msalUiRequiredException = context.Exception as MsalUiRequiredException;
                ...
                if (CanBeSolvedByReSignInUser(msalUiRequiredException))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(ScopeKeySection) && Scopes != null && Scopes.Length > 0)
                        throw new InvalidOperationException(
                            $"Either provide the '{nameof(ScopeKeySection)}' or the '{nameof(Scopes)}' to the 'AuthorizeForScopes'.");
                    ...
                    if (!string.IsNullOrWhiteSpace(ScopeKeySection))
                    {
                        ...
                        if (configuration == null)
                            throw new InvalidOperationException(
                                $"The {nameof(ScopeKeySection)} is provided but the IConfiguration instance is not present in the services collection");

                        Scopes = new[] {configuration.GetValue<string>(ScopeKeySection)};
                    }

                    var properties =
                        BuildAuthenticationPropertiesForIncrementalConsent(Scopes, msalUiRequiredException,
                            context.HttpContext);
        ...
        private bool CanBeSolvedByReSignInUser(MsalUiRequiredException ex)
        {
            ...
            return ex.ErrorCode.ContainsAny(MsalError.UserNullError, MsalError.InvalidGrantError);
        }
            ...
            string[] scopes, MsalUiRequiredException ex, HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(ex.Claims)) properties.Items.Add(OidcConstants.AdditionalClaims, ex.Claims);
